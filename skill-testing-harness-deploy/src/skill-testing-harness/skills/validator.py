"""Deterministic format validator (no LLM)."""
...
def validate(output: str) -> tuple[list[CheckResult], bool, float]:
    ...
    for cid, pat in REQUIRED_HEADINGS:
        ...
        for i in range(cursor, len(flat)):
            if re.match(pat, flat[i].strip()):
                ...
                break
        if found_at is None:
            checks.append(CheckResult(cid, False, f"missing /{pat}/"))
            order_ok = False
        else:
            checks.append(CheckResult(cid, True, f"line {found_at + 1}"))
            cursor = found_at + 1
    checks.append(CheckResult("headings_in_order", order_ok,
                              "ok" if order_ok else "missing or out of order"))

    starts_clean = not output.lstrip().startswith("```")
    checks.append(CheckResult("no_code_fence_wrapper", starts_clean,
                              "no fence" if starts_clean else "wrapped in ``` fence"))

    obj_section = _section(output, r"^##\s*学习目标\s*:", r"^##\s*脚本\s*:")
    obj_bullets = [l for l in obj_section.splitlines() if l.strip().startswith(("-", "•", "*"))]
    checks.append(CheckResult("exactly_3_objectives", len(obj_bullets) == 3,
                              f"found {len(obj_bullets)}"))

    sub_section = _section(output, r"^##\s*字幕要点\s*:", None)
    sub_bullets = [l for l in sub_section.splitlines() if l.strip().startswith(("-", "•", "*"))]
    checks.append(CheckResult("exactly_3_subtitles", len(sub_bullets) == 3,
                              f"found {len(sub_bullets)}"))
    ...
    for cid, body in (("open_nonempty", open_body),
                      ("main_nonempty", main_body),
                      ("close_nonempty", close_body)):
        checks.append(CheckResult(cid, bool(body),
                                  f"{len(body)} chars" if body else "empty"))

    h1_count = sum(1 for l in flat if re.match(r"^#\s+", l.strip()))
    checks.append(CheckResult("single_h1", h1_count == 1, f"{h1_count} H1"))
