...
OpenAI-compatible REST endpoint. So instead of POSTing to the gateway, we
shell into the running `openclaw` container and invoke its built-in CLI:

    docker exec <container> node /app/openclaw.mjs agent \
        --agent <id> --message <prompt> --json --timeout <s>

The CLI authenticates against the gateway using the locally-stored config,
which sidesteps the gateway-token mismatch and works regardless of whether
the gateway is bound to loopback or LAN.
"""
...
def _ensure_docker_available() -> None:
    if shutil.which("docker") is None:
        raise RuntimeError(
            "`docker` CLI not found in the harness container. "
            "Rebuild the harness image (it should install docker-ce-cli)."
        )
    if not os.path.exists("/var/run/docker.sock"):
        raise RuntimeError(
            "/var/run/docker.sock is not mounted into the harness container. "
            "Add it to the harness service in docker-compose.yml."
        )


def _extract_reply(stdout: str) -> str:
    """
    The CLI emits human-readable diagnostic lines plus a final JSON object
    when --json is passed. Find the last JSON object in the output and pull
    the assistant text from it; fall back to the raw stdout.
    """
    text = stdout.strip()
    if not text:
        return ""

    # Scan from the end for a balanced top-level JSON object.
    end = text.rfind("}")
    while end != -1:
        depth = 0
            ...
            if ch == "}":
                ...
                if depth == 0:
                    ...
                    try:
                        ...
                        break
                    # Common shapes the CLI produces: { reply: { content: "..." } }
                    # or { content: "..." } or { message: "..." }.
...
def call_agent(
    ...
    *,
    timeout: int = 600,
    max_retries: int = 2,
    max_tokens: int | None = None,  # accepted for API compatibility, unused
) -> str:
    """
    Run one turn of the named OpenClaw agent and return the assistant text.
    """
